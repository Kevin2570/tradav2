...
const Body = z.object({
  ...
  matchStyle: z.enum(['Strict', 'Open-Ended']),
  ...
  try {
    ...
    if (data.offers.length) {
      await prisma.offer.createMany({
        data: data.offers.map((text) => ({ text, userId: user.id })),
      });
    }
    if (data.wants.length) {
      await prisma.want.createMany({
        data: data.wants.map((text) => ({ text, userId: user.id })),
      });
    }
